...
model ReadRealCSV "Read real matrix from CSV file"
  ...
  parameter Modelica.SIunits.Voltage va[:,1] = KeyWordIO.readRealCSV(csvTabFileName,colBegin,colEnd,1,1,"\t",cache);
  parameter String csvCommaFileName = Modelica.Utilities.Files.loadResource("modelica://KeyWordIO/Resources/comma.csv");
  parameter Modelica.SIunits.Current ia[:,1] = KeyWordIO.readRealCSV(csvCommaFileName,colBegin,colEnd,2,2,",",cache);
  parameter String csvSpaceFileName = Modelica.Utilities.Files.loadResource("modelica://KeyWordIO/Resources/space.csv");
  parameter Modelica.SIunits.Conversions.NonSIunits.AngularVelocity_rpm n[:,1] = KeyWordIO.readRealCSV(csvSpaceFileName,colBegin,colEnd,3,3," ",cache);
  parameter String csvSpace2FileName = Modelica.Utilities.Files.loadResource("modelica://KeyWordIO/Resources/space2.csv");
  parameter Modelica.SIunits.Torque tau[:,1] = KeyWordIO.readRealCSV(csvSpace2FileName,colBegin,colEnd,4,4,"  ",cache);
