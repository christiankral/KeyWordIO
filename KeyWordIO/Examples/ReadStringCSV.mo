...
model ReadStringCSV "Read string matrix from CSV file"
  ...
  parameter String header[1,:] = KeyWordIO.readStringCSV(csvTabFileName,rowHeader,rowHeader,1,4,delimiter="\t",cache=cache);
  parameter String units[1,:] = KeyWordIO.readStringCSV(csvTabFileName,rowUnits,rowUnits,1,4,delimiter="\t",cache=cache);
equation
  when terminal() then
